...
template <typename T>
void calc_cofactors (int const keldysh_idx, int const size, T &wick_mat_1, T &wick_mat_2, T &minors){

  // Calculate LU decomposition with partial pivoting
  triqs::arrays::vector<int> pivot_index_array(size + 1);
  int info = triqs::arrays::lapack::getrf(wick_mat_1, pivot_index_array, true);
  if (info != 0) {
  ...
  minors() = 0;
  minors(0, 0) = g_mat_s1_det;

  info = triqs::arrays::lapack::getrs(wick_mat_1, minors, pivot_index_array);
  if (info != 0) {
    TRIQS_RUNTIME_ERROR << "lapack::getrs failed with code " << info;
  }

  // Multiply by Keldysh index parity & other spin determinant:
  minors *= GetBitParity(keldysh_idx) * triqs::arrays::determinant(wick_mat_2);
}
...
std::pair<binner::sparse_binner_t<1, 1>, int> integrand_g_kernel::operator()(std::vector<double> const &times,
  ...
  binner::sparse_binner_t<1, 1> result{};
