...
void bin_values(hist_xi& xi, int axis, 
				std::vector<std::vector<double>> points, std::vector<double> values) {
 bool in_range;
...
struct lorentz {
	double operator()(double t) {return 1.0 / ((1.0 + t) * (1.0 + t));}
};
...
class warper_plasma_projection_t {
 ...
 public: 
 warper_plasma_projection_t(std::function<double(std::vector<double>)> integrand_, warper_plasma_1D_t w_warper_, int order, int nr_function_sample_points) 
 : integrand(std::move(integrand_)), w_warper(w_warper_), order(order){
    ...
    w_vals.push_back( std::abs(1.0/w_warper.evaluate_warping_function(u)) );
	}
