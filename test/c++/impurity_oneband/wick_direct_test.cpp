...
TEST(integrand_direct, Values) { // NOLINT
  ...
  for (keldysh_idx_t a : {forward, backward}) {
    ...
    expected_val += sign * g0_k({1.0, down, forward}, {1.0, down, backward})
       * (g0_k({1.0, up, forward}, {1.0, up, backward}) * g0_k(external_A, external_B)
          - g0_k(external_A, A) * g0_k(A, external_B));
  }
  ...
  EXPECT_NEAR(std::imag(expected_val), std::imag(computed_val), 1e-16); // absolute tolerance because we compute 0
  ...
  for (keldysh_idx_t a : {forward, backward}) {
    for (keldysh_idx_t b : {forward, backward}) {
      ...
      A = {1.0, up, a};
      B = {2.0, up, b};
       det_up = (g0_k({1.0, up, forward}, {1.0, up, backward}) * g0_k({2.0, up, forward}, {2.0, up, backward}) - g0_k(B, A) * g0_k(A, B)) * g0_k(external_A, external_B)
